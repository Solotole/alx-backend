...
class LFUCache(BaseCaching):
    ...
    def __init__(self):
        ...
        super().__init__()
        self.access_order = []  # List to maintain the order of key access
        self.key_access_time = {}  # Dictionary to track the last access time of each key
        self.frequency_count = {}  # Dictionary to track the frequency of each key's usage

    def put(self, key, item):
        """Assign the item value to the key in the cache, following LFU policy."""
        ...
        if len(self.cache_data) >= BaseCaching.MAX_ITEMS:
            if key not in self.cache_data:
                ...
                lfu_key = min(self.frequency_count, key=self.frequency_count.get)
                
                # If multiple keys have the same frequency, evict the least recently used one
                candidates = [k for k, v in self.frequency_count.items() if v == self.frequency_count[lfu_key]]
                if len(candidates) > 1:
                    lfu_key = min(candidates, key=self.key_access_time.get)
                
                # Remove the least frequently used (and least recently used if tied) key
